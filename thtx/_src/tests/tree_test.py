...
def tree_to_pytree(tree: thtx.Tree, batch_i: int = 0):
...
class TreeTest(parameterized.TestCase):
  ...
  def _reproduce_tree(self, tree):
    ...
    policy_fn = dict(
        gumbel_muzero=thtx.gumbel_muzero_policy,
        muzero=thtx.muzero_policy,
    )[tree["algorithm"]]
    ...
    qtransform = functools.partial(
        getattr(thtx, tree["algorithm_config"].pop("qtransform")),
